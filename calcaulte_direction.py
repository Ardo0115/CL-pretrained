...
def main():
    ...
    for t, ncla in task_info:
        ...
        # train_loader = train_dataset_loaders[0]
        # train_iterator = torch.utils.data.DataLoader(train_loader, 100, shuffle=False)
        # conv1_grad_for_task0 = compute_gradient(model2, train_iterator, device, 0)['conv1.weight']
        
        # train_loader = train_dataset_loaders[1]
        # train_iterator = torch.utils.data.DataLoader(train_loader, 100, shuffle=False)
        # conv1_grad_for_task1 = compute_gradient(model2, train_iterator, device, 1)['conv1.weight']
        
        cos = nn.CosineSimilarity(dim=0, eps = 1e-6)

        assert direction.keys() == grad_for_task0.keys()
        assert direction.keys() == grad_for_task1.keys()

        # f = open("{}/cosine_similarity.txt".format(os.getcwd()), 'w')
        # for n, p in model2.named_parameters():
        #     cos_with0 = cos(direction[n].view(-1), grad_for_task0[n].view(-1))
        #     cos_with1 = cos(direction[n].view(-1), grad_for_task1[n].view(-1))
        #     cos_with01 = cos(direction[n].view(-1), (grad_for_task0[n]+grad_for_task1[n]).view(-1))
        #     f.write("{} : {}\t {}\t {}\n".format(n,cos_with0, cos_with1, cos_with01))

        f = open("{}/norm.txt".format(os.getcwd()), 'w')
        for n, p in model2.named_parameters():
            cos_with0 = torch.norm(grad_for_task0[n])
            cos_with1 = torch.norm(grad_for_task1[n])
            f.write("{} : {}\t {}\n".format(n,cos_with0, cos_with1))
            
            # print(cos(conv1_grad_for_task0.view(-1), conv1_grad_for_task1.view(-1)))
            # print(cos(direction.view(-1), (conv1_grad_for_task0+conv1_grad_for_task1).view(-1)))
            # print(cos(direction.view(-1), (conv1_grad_for_task0).view(-1)))
            # print(cos(direction.view(-1), (conv1_grad_for_task1).view(-1)))
        f.close()
