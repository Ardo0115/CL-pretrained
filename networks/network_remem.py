...
class BasicBlock_PiggyBack(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, mask_init, mask_scale, threshold_fn):
        ...
        self.elementwiseconv1 = torch.nn.ModuleList()
        for t,_ in self.task_info:
            self.elementwiseconv1.append(nl.ElementWiseConv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False,mask_init=mask_init, mask_scale=mask_scale, threshold_fn=threshold_fn))
        self.batchnorm1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)

        self.elementwiseconv2 = torch.nn.ModuleList()
        for t,_ in self.task_info:
            self.elementwiseconv2.append(nl.ElementWiseConv2d(out_channels, out_channels * BasicBlock_PiggyBack.expansion, kernel_size=3, padding=1, bias=False, mask_init=mask_init, mask_scale=mask_scale, threshold_fn=threshold_fn))

        self.batchnorm2 = nn.BatchNorm2d(out_channels * BasicBlock_PiggyBack.expansion)
        '''
        #residual function
        self.residual_function = nn.Sequential(
            self.elementwiseconv1,
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            self.elementwiseconv2,
        ...
        #shortcut
        #self.shortcut = nn.Sequential()
        self.elementwiseconv3 = torch.nn.ModuleList()
        for t,_ in self.task_info:
            self.elementwiseconv3.append(nn.Sequential())
        self.batchnorm3 = nn.Sequential()
        ...
        if stride != 1 or in_channels != BasicBlock_PiggyBack.expansion * out_channels:
            self.elementwiseconv3 = torch.nn.ModuleList()
            for t,_ in self.task_info:
                self.elementwiseconv3.append(nl.ElementWiseConv2d(in_channels, out_channels * BasicBlock_PiggyBack.expansion, kernel_size=1, stride=stride, bias=False, mask_init=mask_init, mask_scale=mask_scale, threshold_fn=threshold_fn))
            self.batchnorm3 = nn.BatchNorm2d(out_channels * BasicBlock_PiggyBack.expansion)

    def forward(self, x, task_t):
        x_residual = self.elementwiseconv1[task_t](x)
        x_residual = self.batchnorm1(x_residual)
        x_residual = self.relu(x_residual)
        x_residual = self.elementwiseconv2[task_t](x)
        x_residual = self.batchnorm2(x_residual)

        x_shortcut = self.elementwiseconv3[task_t](x)
        x_shortcut = self.batchnorm3(x_shortcut)

        return nn.ReLU(inplace=True)(x_residual + x_shortcut)
...
class ResNet_PiggyBack(nn.Module):
    ...
    def __init__(self, task_info, block, num_block, mask_init, mask_scale, threshold_fn, num_classes=100, init_weights=True):
        ...
        self.elementwiseconv1 = torch.nn.ModuleList()
        for t,_ in self.task_info:
            self.elementwiseconv1.append(nl.ElementWiseConv2d(3, 64, kernel_size=3, padding=1, bias=False, mask_init=mask_init, mask_scale=mask_scale, threshold_fn=threshold_fn))

        self.conv1 = nn.Sequential(
                self.elementwisevonv1,
                nn.BatchNorm2d(64),
                nn.ReLU(inplace=True))
        #we use a different inputsize than the original paper
        #so conv2_x's stride is 1
