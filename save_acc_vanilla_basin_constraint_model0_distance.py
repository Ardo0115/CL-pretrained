...
def main():
    ...
    far_model_task1.load_state_dict(torch.load('./trained_model/_CIFAR100_for_Resnet_vanilla_basin_constraint_from_model1_SGD_0_lamb_{}_lr_0.1_batch_256_epoch_100_task_0.pt'.format(distance_lamb)))
    ...
    for t, ncla in task_info:
        ...
        for u in range(t+1):
            ...
            if t==0:
                ...
                break
            print(f"Distance from finetuned to far model_{t} : {compute_distance(model1, model2)}")
        ...
        for task_t, acc_loss in enumerate(zip(acc, lss)):
            ...
            np.savetxt('vanilla_basin_constraint_distance_lamb_{}.txt'.format(distance_lamb), acc_t, '%.4f')
            break
