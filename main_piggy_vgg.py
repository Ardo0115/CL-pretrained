...
def main():
    ...
    if args.output == '':
        args.output = './result_data_vgg16/' + log_name + '.txt'
    ########################################################################################################################
    # Seed
    ...
    if not os.path.isdir('result_data_vgg16'):
        print('Make directory for saving results')
        os.makedirs('result_data_vgg16')
    ...
    for t, ncla in task_info:
        ...
        for u in range(t+1):
            ...
            if u == t:
                ...
                #    if n and (test_model.state_dict()[n].data - early_model[n_curr].data).abs().sum() >1e-8:
                #for n, n_curr in zip(test_model.state_dict().keys(), myTrainer.model.state_dict().keys()):
                #    if n and (test_model.state_dict()[n].data - myTrainer.model.state_dict()[n_curr].data).abs().sum() >1e-8:
