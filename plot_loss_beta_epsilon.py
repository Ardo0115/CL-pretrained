...
def main():
    ...
    for filename in os.listdir(target_dir):
        if filename.endswith(".npy"):
            ...
            beta2 = re.search('beta2_(.+?)_', filename)
            if beta2 is not None:
                beta2 = beta2.group(1)
            eps = re.search('eps_(.+?).npy', filename).group(1)
            loss_list.append((tilt, beta2, eps, np.load(os.path.join(target_dir, filename))))
        else:
            continue
...
# Plot in one Figure
    if plot_type == 1:
        ...
        for tilt, beta2, eps, loss_arr in loss_list:
            ...
            plt.plot(loss_arr, label='tilt={}, beta2={}, eps={}'.format(tilt, beta2, eps))
        plt.legend()
