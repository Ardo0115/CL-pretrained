...
def main():
    ...
    if args.output == '':
        args.output = './result_data_again/' + log_name + '.txt'
    ########################################################################################################################
    # Seed
    ...
    if not os.path.isdir('result_data_again'):
        print('Make directory for saving results')
        os.makedirs('result_data_again')
    ...
    optimizer = torch.optim.SGD(myModel.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)
    ...
    for t, ncla in task_info:
        ...
        if t==0:
            if args.lamb == 0.0 and args.lr == 0.1:
