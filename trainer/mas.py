...
class Trainer(trainer.GenericTrainer):
    ...
    def train(self, train_loader, test_loader, t, device = None):
        ...
        if t == 0:
            ...
            self.update_frozen_model()
            self.update_omega()
            #print("Omeag : {}".format(torch.sum(self.omega['conv1.0.weight'])))
        ...
        for samples in tqdm(self.fisher_iterator):
            ...
            for n,p in self.model.named_parameters():
                if p.grad is not None:
                    omega[n]+=batch_size*p.grad.data.clone().abs_()
        #f = open("{}/knowledge_omega/task_{}_knowledge_{}.txt".format(os.getcwd(),self.t, self.knowledge_ratio), 'a')
        # Mean
        with torch.no_grad():
