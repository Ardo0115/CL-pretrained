...
class Trainer(trainer.GenericTrainer):
    ...
    def criterion(self,output,targets):
        ...
        new_conv_params = self.get_conv_params(self.model)
        old_conv_params = self.get_conv_params(self.model_fixed)
        loss_reg = 0
        loss_reg2 = 0
        if self.t > 1:
            film_param_diff = new_film_params - old_film_params
            conv_param_diff = new_conv_params - old_conv_params
            film_fisher_param_diff_mul = torch.matmul(self.film_fisher, film_param_diff)

            loss_reg += torch.matmul(film_param_diff, film_fisher_param_diff_mul) / 2.0
            # more lambda should be multiplied for conv fisher
            loss_reg2 += torch.sum(self.conv_fisher * conv_param_diff.pow(2)) / 2.0
        ...
        return self.ce(output,targets)+self.lamb*loss_reg + self.lamb2*loss_reg2
    ...
    def compute_film_fisher(self):
        ...
        for samples in tqdm(self.fisher_iterator):
            ...
            conv_params_grad = torch.Tensor().to(self.device)
            # Get gradients
            for n,p in self.model.named_parameters():
                if "film" in n and p.grad is not None:
                    film_params_grad = torch.cat([film_params_grad, p.grad.view(-1)])
                if ("conv4" in n or "batchnorm4" in n) and p.grad is not None:
                    conv_params_grad = torch.cat([conv_params_grad, p.grad.view(-1)])
            film_fisher += batch_size**2 * torch.outer(film_params_grad, film_params_grad)
            conv_fisher += batch_size**2 * conv_params_grad**2
            

        with torch.no_grad():
            film_fisher /= len(self.fisher_iterator.dataset)
            conv_fisher /= len(self.fisher_iterator.dataset)
    ...
    def update_fisher(self):
        ...
        if self.t>1:
            ...
            conv_fisher_old = (self.conv_fisher).clone()
        ...
        if self.t>1:
            ...
            self.conv_fisher=(self.conv_fisher+conv_fisher_old*(self.t-1))/(self.t)
