...
class Trainer(trainer.GenericTrainer):
    ...
    def train(self, train_loader, test_loader, t, device = None):
        ...
        for epoch in range(self.args.nepochs):
            ...
            print()
            #if t > 0:
            #    assert torch.equal(self.model.state_dict()['batchnorm1.running_mean'], self.model_fixed.state_dict()['batchnorm1.running_mean'])
            #    for (n, p), (n_fixed, p_fixed) in zip(self.model.named_parameters(), self.model_fixed.named_parameters()):
            #        if 'last.0.weight' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'last.0.bias' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'conv1.weight' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'conv1.bias' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'batchnorm1.weight' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'batchnorm1.bias' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'batchnorm2.weight' in n:
            #            assert torch.equal(p_fixed, p)
            #        if 'batchnorm' in n:
            #            assert torch.equal(p_fixed, p)
                    #if 'batchnorm2.bias' in n:
                    #    assert torch.equal(torch.zeros(p.size()).to(self.device), p)
