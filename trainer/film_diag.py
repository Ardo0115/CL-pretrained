...
class Trainer(trainer.GenericTrainer):
        ...
        for samples in tqdm(self.fisher_iterator):
            ...
            for n,p in self.model.named_parameters():
                if "film" in n and p.grad is not None:
                    film_params_grad = torch.cat([film_params_grad, p.grad.view(-1).clone()])
            fisher += batch_size**2 * film_params_grad.clone().pow(2)
    ...
    def update_fisher(self):
        ...
        if self.t>1:
            self.fisher=(self.fisher+fisher_old*(self.t-1))/(self.t)
