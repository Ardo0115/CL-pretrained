        ...
        if self.t > 0:
            for module, module_old in zip(self.model.modules(), self.model_fixed.modules()):
                if 'Conv' in str(type(module)):
                    ...
                    module.weight.data = (curr_weight + module_old.weight.data) / 2.0
                    if module.bias is not None:
                        curr_bias = module.bias.data
                        module.bias.data = (curr_bias + module_old.bias.data) / 2.0
                elif 'BatchNorm' in str(type(module)):
                    curr_weight = module.weight.data
                    curr_bias = module.bias.data
                    ...
                    module.weight.data = (curr_weight + module_old.weight.data) / 2.0
                    module.bias.data = (curr_bias + module_old.bias.data) / 2.0
                    module.running_mean.data = (curr_running_mean + module_old.running_mean.data) / 2.0
                    module.running_var.data = (curr_running_var + module_old.running_var.data) / 2.0
